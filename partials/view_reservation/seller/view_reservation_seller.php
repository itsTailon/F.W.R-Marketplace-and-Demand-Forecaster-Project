...
<script>
    ...
    document.getElementById('no-show-button').addEventListener('click', () => {
        $.ajax({
            type: 'POST',
            url: '/backend/API/Model/sellerReservation.php',
            data: {
            ...
            success: function() {
                alert("Reservation successfully marked no-show!");
                window.location.href = '/active_reservations.php';
            },
            error: function(err) {
                statusMessage.textContent = 'Failed to mark as no-show: ' + err.status;
            }
